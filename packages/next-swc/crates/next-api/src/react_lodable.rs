    ...
    */
    ...
    for import_mapping in import_mappings {
        ...
        for (import, module) in import_sources {
            ...
            //let evaluated = client_chunking_context.chunk_group(Vc::upcast(entry));
            ...
            for ee in evaluated {
                ...
                // 1. replace entry to top level, real entry instead of chunks
    ...
    let mut x = NonDeterministic::new()
        .skip_duplicates()
        .visit([Vc::upcast(entry)], get_referenced_modules)
        .await
        .completed()?
    ...
    println!("first--------- {:#?}", x.first().is_some());

    // temp only

    let em = if let Some(t) = x.first().take() {
        //let x = DynamicImportsMap((t.0 .0.clone(), t.0 .1.clone()));
        //OptionDynamicImportsMap(Vc::cell(Some(Vc::cell((t.0 .0.clone(), t.0 .1.clone())))))
        Vc::cell(Some(Vc::cell(((t.0 .0.clone(), t.0 .1.clone())))))
    } else {
        OptionDynamicImportsMap::none()
    };
    Ok(em)
}
